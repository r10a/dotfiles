-- copyreference: stage `path:line` references (with code + per-item prompt)
-- into a review and copy them together, formatted for pasting to LLMs.
...
function M.copy_all()
    if #staged == 0 then
        vim.notify("copyreference: nothing staged", vim.log.levels.WARN)
        return
    end
